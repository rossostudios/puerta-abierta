...
ADMIN_HOSTNAMES="${ADMIN_HOSTNAMES:-app.casaora.co}"
...
aws_cmd ec2 authorize-security-group-ingress \
  --group-id "${ecs_sg_id}" \
  --ip-permissions "[{\"IpProtocol\":\"tcp\",\"FromPort\":${CONTAINER_PORT},\"ToPort\":${CONTAINER_PORT},\"UserIdGroupPairs\":[{\"GroupId\":\"${alb_sg_id}\",\"Description\":\"ALB to admin ECS\"}]}]" \
  >/dev/null 2>&1 || true
...
jq -n \
  ...
  '{
    ...
    note: "Create or update Cloudflare CNAME(s) to the ALB DNS name, then deploy the admin ECS service into this target group."
  }'
