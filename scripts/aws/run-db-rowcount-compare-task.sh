...
TASK_FAMILY="${TASK_FAMILY:-casaora-db-compare}"
...
LOG_GROUP_NAME="${LOG_GROUP_NAME:-/ecs/casaora-db-compare}"
TABLES_CSV="${TABLES_CSV:-app_users,organizations,organization_members,properties,units,reservations,agent_approvals,knowledge_documents,knowledge_chunks,agent_traces}"
...
compare_command="$(cat <<'SH'
set -euo pipefail
SRC_DB="$SOURCE_DATABASE_URL"
DST_DB="$TARGET_DATABASE_URL"
tables="${TABLES_CSV}"
echo "public_table_count|source|$(psql "$SRC_DB" -Atqc "select count(*) from information_schema.tables where table_schema='public' and table_type='BASE TABLE'")"
echo "public_table_count|target|$(psql "$DST_DB" -Atqc "select count(*) from information_schema.tables where table_schema='public' and table_type='BASE TABLE'")"
for t in $(printf '%s' "$tables" | tr ',' ' '); do
  src_count="$(psql "$SRC_DB" -Atqc "select count(*) from public.${t}")"
  dst_count="$(psql "$DST_DB" -Atqc "select count(*) from public.${t}")"
  echo "row_count|${t}|source=${src_count}|target=${dst_count}"
done
SH
)"
...
jq \
  ...
  --arg cmd_script "${compare_command}" \
  ...
  '
    ...
    | .containerDefinitions[0].environment = [{"name":"TABLES_CSV","value":$tables_csv}]
...
echo "==> Waiting for compare task to stop: ${task_arn}"
