-- Phase 9: runtime rollout controls, shadow parity storage, and trace metadata.
...
DROP POLICY IF EXISTS agent_runtime_rollouts_read ON agent_runtime_rollouts;
CREATE POLICY agent_runtime_rollouts_read
...
DROP POLICY IF EXISTS agent_runtime_rollouts_manage ON agent_runtime_rollouts;
CREATE POLICY agent_runtime_rollouts_manage
...
DROP POLICY IF EXISTS agent_runtime_parity_runs_read ON agent_runtime_parity_runs;
CREATE POLICY agent_runtime_parity_runs_read
...
DROP POLICY IF EXISTS agent_runtime_parity_runs_manage ON agent_runtime_parity_runs;
CREATE POLICY agent_runtime_parity_runs_manage
  ...
  USING (
    organization_id::text IN (
      ...
      FROM organization_members
      WHERE user_id = auth.uid()
        AND role = ANY (ARRAY['owner_admin', 'operator', 'accountant'])
    )
  )
  WITH CHECK (
    organization_id::text IN (
